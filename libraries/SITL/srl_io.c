...
int SIO_open(unsigned short arg_ChannelNum, SIO_Baud argBaud,SIO_DataBits argDataBits,SIO_StopBits argStopBits,SIO_parity arg_parity)
{
  ...
  if(arg_ChannelNum == PORT0) // connection to STM via rs232 , oil.dat
  {
    sprintf(devName,"/dev/ttyUSB0",arg_ChannelNum);
    printf("Opening port %s\n",devName);
    mx_fd[arg_ChannelNum] = open(devName,O_RDWR);
//#ifndef OBC_IN_LOOP_BLOCK     // IMPORTANT: MAKE SURE ONLY OBC IN LOOP PORT IS MADE BLOCKING, NOT TELEMETRY, BY DEFAULT EVERY PORT IS BLOCKING MODE
    // fcntl(mx_fd[arg_ChannelNum],F_SETFL,fcntl(mx_fd[arg_ChannelNum],F_GETFL,0)|O_NONBLOCK);
//#else
//	printf("blocking port 0\n");
//	getchar();
//#endif
  }
//  if (arg_ChannelNum == PORT3) // telemetry.dat,  receive logging data via stm uart
//  {
//    sprintf(devName,"/dev/ttyAP9",arg_ChannelNum);
//    printf("Opening port %s\n",devName);
//    mx_fd[arg_ChannelNum] = open(devName,O_RDWR);
//    fcntl(mx_fd[arg_ChannelNum],F_SETFL,fcntl(mx_fd[arg_ChannelNum],F_GETFL,0)|O_NONBLOCK);
//  }
//  if (arg_ChannelNum == PORT1)
//  {
//    sprintf(devName,"/dev/ttyAP10",arg_ChannelNum);
//    printf("Opening port %s\n",devName);
//    mx_fd[arg_ChannelNum] = open(devName,O_RDWR);
//    //mx_fd[arg_ChannelNum] = open(devName,O_WRONLY);
//  }
