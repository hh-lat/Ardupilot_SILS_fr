#!/usr/bin/env python3
"""
auto_flight2.py  -  Full scripted ArduPlane SITL mission for the custom uSTOL FDM.

Where auto_flight.py does a minimal takeoff -> FBWA -> RTL, this script flies a
complete mission profile from a single run, records full flight telemetry, and
reports STOL performance metrics.  You are asked for the mission parameters at
the start; the script then flies, in order:
...
then in another terminal:
    ...
    ./Tools/autotest/auto_flight2.py                 # interactive
    ./Tools/autotest/auto_flight2.py --log flight.csv # also record CSV
    ./Tools/autotest/auto_flight2.py --defaults       # all defaults, no prompts
...
def apply_setup_params(plane, p):
    ...
    if p.apply_reversals:
        # uSTOL FDM sign convention (see auto_flight.py): without these the
        # auto take-off drives the nose DOWN, rolls off, and yaws into a spiral.
