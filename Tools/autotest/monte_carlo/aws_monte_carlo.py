#!/usr/bin/env python3
"""
Monte Carlo Runner — uSTOL FBWA OPEN-LOOP PROFILE (mission_ustol_2)
===================================================================
...
Usage:
    python3 aws_monte_carlo.py --runs 10 --workers 10
    python3 aws_monte_carlo.py --runs 500 --workers 30 --speedup 10
"""
...
def fly_ustol2(conn, m, writer, st, perf, sitl_proc, t0_flight, deadline):
    """Fly the mission_ustol_2 open-loop FBWA profile via RC overrides.
    ...
    Returns : 'cruise_hold' | 'crashed_climb' | 'crashed_cruise' | 'climb_timeout'
    """
    ...
    while True:
        ...
        if st["alt"] >= CRUISE_ALT - LEVEL_BAND:
            # Flare: smoothstep pitch -> 0 AND throttle full -> cruise-trim over LEVEL_BAND.
            if flare0 is None:
                flare0 = theta
            s = _smooth((st["alt"] - (CRUISE_ALT - LEVEL_BAND)) / LEVEL_BAND)
            target  = flare0 * (1.0 - s)
...
def metrics_from_csv(csv_path: str, home_lat: float, home_lon: float) -> dict:
    ...
    m = dict(max_alt_m=0.0, max_roll_deg=0.0, max_pitch_deg=0.0,
             ...
             landing_lat=None, landing_lon=None, landing_err_m=None,
...
def write_case_report(filepath, case_id, perturbed, nominals, result):
    with open(filepath, "w") as f:
        f.write(f"Monte Carlo Case #{case_id:04d}  [uSTOL FBWA open-loop profile]\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"  Reached hold      : {result.get('mission_complete', False)}\n")
...
def _run_case(case_id, case_seed, instance_id, config, perturbed, output_dir,
              workspace, speedup, save_plot, hard_timeout):
    """Execute one Monte Carlo case: launch SITL, fly the uSTOL FBWA open-loop profile."""
    result = dict(
    ...
    try:
        ...
        result["mission_complete"] = (land_result == "cruise_hold")
        result["ground_roll_m"]    = perf["ground_roll_m"]
        result["liftoff_speed"]    = perf["liftoff_speed"]
        result["landing_roll_m"]   = None                 # no landing in this profile
...
def _worker_entry(args):
    (case_id, case_seed, config, perturbed, output_dir, workspace,
     speedup, save_plot, hard_timeout) = args
...
def main():
    parser = argparse.ArgumentParser(
        description="Monte Carlo Runner — uSTOL FBWA open-loop profile (mission_ustol_2)")
    ...
    parser.add_argument("--hard-timeout", type=float, default=None,
                        help="seconds before a stuck case's SITL is force-killed "
                             "(default: config mission.hard_timeout_s or 900)")
    ...
    hard_timeout = (args.hard_timeout
                    or config["mission"].get("hard_timeout_s", 900))

    print("=" * 70)
    print("  LAT Monte Carlo Runner — uSTOL FBWA open-loop profile (mission_ustol_2)")
    print("=" * 70)
    ...
    print("  Profile       : GROUND→CLIMB→FLARE→CRUISE-HOLD  (open-loop FBWA, no landing)")
    print("=" * 70)
    ...
    tasks = [(cid, f"{args.seed}:{cid}", config, all_perturbed[cid], output_dir,
              workspace, args.speedup, args.plot, hard_timeout)
    ...
    with ProcessPoolExecutor(max_workers=max_workers,
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                ms = "HOLD" if res["mission_complete"] else res["land_result"]
    ...
    with open(done_path, "w") as f:
        ...
        f.write(f"Reached hold    : {n_ms}/{n_tot}\n")
