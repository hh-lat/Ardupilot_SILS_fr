...
for ii = 1:N
    ...
    [beta_loo, ~, ~] = logreg_irls(X_tr, y_tr, w_tr, lambda_log, 500, 1e-8);
...
for rr = 1:n_repeats
    ...
    for kk = 1:k_folds
        ...
        [beta_k, ~, ~] = logreg_irls(X_lr(train_mask,:), y(train_mask), ...
            w_class(train_mask), lambda_log, 500, 1e-8);
...
%LOGREG_IRLS  L2-regularised logistic regression via IRLS.
    ...
    for it = 1:max_iter
        ...
        % Per-component step cap
        delta = max(min(delta, 5.0), -5.0);
