#!/usr/bin/env python3
"""
Monte Carlo Parallel Runner for LAT SITL FDM
=============================================
Runs N Monte Carlo cases with CPU parallelism.

Usage:
    python monte_carlo_runner.py                          # uses defaults
    python monte_carlo_runner.py --config my_config.json  # custom config
    python monte_carlo_runner.py --workers 8 --runs 50    # override

Each SITL instance is launched as a standalone `arduplane` binary with a
unique instance ID, override file (via LAT_MC_OVERRIDE_FILE env var), and
working directory.  pymavlink is used to upload the mission, arm, and
monitor the flight.

Mission: TAKEOFF → GUIDED to nearby point → LOITER until timeout (no landing)
"""
...
def write_case_report(filepath: str, case_id: int,
    ...
    with open(filepath, "w") as f:
        f.write(f"Monte Carlo Case #{case_id:04d}\n")
        f.write("=" * 65 + "\n\n")
...
def build_mission(home_lat, home_lon, home_alt, takeoff_alt):
    ...
    After takeoff, the monitor loop guides the aircraft to a nearby
    point and switches to LOITER until timeout.  No landing waypoint.
    """
...
def wait_prearm_ok(conn, timeout: float = 90):
    ...
    while time.time() - t0 < timeout:
        ...
        if msg is not None:
            # onboard_control_sensors_health bit 0x04 = pre-arm healthy
            # Simpler: try to read STATUSTEXT for pre-arm pass
            pass
        # Also drain STATUSTEXT looking for "Ready to fly"
        ...
        if txt:
            ...
            if 'ready' in text_str.lower() or 'prearm' not in text_str.lower():
                pass  # keep waiting for positive confirmation
        time.sleep(0.5)
...
def arm_vehicle(conn, timeout: float = 60.0) -> bool:
    ...
    while time.time() - t0 < timeout:
        ...
        if attempt <= 4:
            ...
            # Force arm (param2 = 21196 is the magic force value)
            conn.mav.command_long_send(
                conn.target_system, conn.target_component,
                mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM,
                0, 1, 21196, 0, 0, 0, 0, 0)  # force arm

        # Wait for ACK or check heartbeat
        for _ in range(5):
            hb = conn.recv_match(type="HEARTBEAT", blocking=True, timeout=2)
...
def _run_case(case_id: int, instance_id: int, config: dict,
    ...
    """
    Execute one Monte Carlo case.  Returns a result dict.
    Called inside a worker process.
    """
    result = dict(
        case_id=case_id, takeoff_success=False, mission_complete=False,
        duration_s=0.0, max_alt_m=0.0, exit_reason="unknown")
    ...
    # ---- Optional GCS output (e.g. Mission Planner) ----
    ...
    try:
        ...
        # Request data streams so we receive telemetry
        request_data_streams(conn)

        # Disable ARSPD pre-arm check for SITL headless runs
        set_param(conn, 'ARMING_CHECK', 0)
        _log(case_id, "ARMING_CHECK disabled")

        # Ensure max throttle during takeoff (PWM 1900 = throttle 1.0)
        ...
        # ---- Open state-log CSV for this case ----
        ...
        # Running state dict — updated by each MAVLink message type
        st = {
            "mode": -1,
...
def _wait_ekf_ready(conn, timeout: float = 60):
    ...
    while time.time() - t0 < timeout:
        ...
        if msg is not None:
            # flags: bits 0-4 set = velocity/pos/height OK
            if (msg.flags & 0x1F) == 0x1F:
                return
    # If timeout, continue anyway — arming may still succeed.
...
# ===================================================================
#  Instance-pool wrapper (for ProcessPoolExecutor)
# ===================================================================
# Each concurrent worker grabs a unique SITL instance-ID from a shared
# queue and returns it when done, ensuring no port collisions.

_instance_queue = None          # set in worker initializer
...
def main():
    ...
    output_dir = str(SCRIPT_DIR / f"mc_results_{ts}")
    os.makedirs(output_dir, exist_ok=True)
    print(f"\n  Output → {output_dir}\n")

    # ---- Sample all cases up-front (reproducible) ----
    ...
    with ProcessPoolExecutor(max_workers=max_workers,
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                _log(cid, f"{status}  |  {res['exit_reason']}  "
                ...
                results.append(dict(
                    ...
                    duration_s=0, exit_reason=f"worker_exception: {exc}"))
