#!/usr/bin/env python3
"""
Monte Carlo Runner — uSTOL ALL-FBWA PROFILE w/ LANDING + WIND (mission_ustol_7)
==================================================================================
Identical to aws_monte_carlo_4.py (all-FBWA takeoff→cruise→approach→POWERED-flare
landing, NO loiter) but each case is flown under ONE selectable WIND condition:
...
Pick the wind TYPE by editing WIND_CASE in the config block below (NO CLI flag).
The wind SPEED sweeps across WIND_SPEED_LEVELS, assigned per case (case i uses
levels[i % len]).  Wind type/speed/direction are recorded per case in result.json
and summary.csv, and the campaign output dir is tagged with the case name.
...
Usage:
    # 1) edit WIND_CASE below, e.g. WIND_CASE = "head"
    # 2) run a campaign (output dir auto-tagged mc_<ts>_<case>):
    python3 aws_monte_carlo_5.py --runs 10 --workers 10
    python3 aws_monte_carlo_5.py --runs 500 --workers 30 --speedup 10
"""
...
# ===========================================================================
#  WIND CASE — EDIT THESE BEFORE RUNNING (no CLI flag)
# ===========================================================================
#  One wind TYPE per campaign; the SPEED sweeps across the levels below so a
#  single run maps degradation vs wind strength.  Directions are relative to
#  the runway heading (config mission.runway_heading_deg) — the aircraft flies
#  straight along it the whole profile, so head/tail/cross are well-defined.
WIND_CASE             = "cross"          # "head"|"tail"|"cross"|"up"|"down"|"none"
WIND_SPEED_LEVELS     = [1.0, 2.0, 3.0] # m/s; case i -> levels[i % len]
CROSSWIND_SIDE        = "right"         # "right" (DIR=hdg+90) | "left" (DIR=hdg-90)
...
def wind_params_for_case(case: str, runway_hdg: float, speed: float,
                         side: str = "right") -> dict:
    """Map a wind case name + speed to the SITL SIM_WIND_* params (+ metadata).

    Convention (verified against SIM_Aircraft.cpp update_wind + the wind_ef sign
    flip at line 990, with wind_ned := wind_ef so V_air = V_gnd - wind_ned):
      ...
      • SIM_WIND_DIR_Z = +90  -> updraft (air rises, reduces sink);
        DIR_Z = -90           -> downdraft (air sinks, increases sink).
    For pure-vertical cases the horizontal component is cos(90)=0, so the whole
    SIM_WIND_SPD magnitude goes vertical.
    """
    case = case.lower()
    if case == "none" or speed <= 0.0:
        spd, direction, dir_z = 0.0, 0.0, 0.0
    elif case == "head":
        spd, direction, dir_z = speed, runway_hdg % 360.0, 0.0
    elif case == "tail":
        spd, direction, dir_z = speed, (runway_hdg + 180.0) % 360.0, 0.0
    elif case == "cross":
        off = 90.0 if side.lower() == "right" else -90.0
        spd, direction, dir_z = speed, (runway_hdg + off) % 360.0, 0.0
    elif case == "up":                       # updraft — rising air
        spd, direction, dir_z = speed, runway_hdg % 360.0, 90.0
    elif case == "down":                     # downdraft — sinking air
        spd, direction, dir_z = speed, runway_hdg % 360.0, -90.0
    else:
        raise ValueError(f"unknown WIND_CASE {case!r} "
                         "(use head|tail|cross|up|down|none)")
    return {
        "SIM_WIND_SPD":   spd,
...
def write_case_report(filepath, case_id, perturbed, nominals, result):
    with open(filepath, "w") as f:
        ...
        f.write(f"Wind      : {result.get('wind_case', 'none')}  "
                f"{result.get('wind_speed_mps', 0):.1f} m/s  "
...
def _run_case(case_id, case_seed, instance_id, config, perturbed, output_dir,
    ...
    """Execute one Monte Carlo case: launch SITL, set the case's wind, fly the
    uSTOL all-FBWA + landing profile (no loiter)."""
    result = dict(
    ...
    try:
        ...
        _log(case_id, f"wind: {wind['wind_case']}  "
                      f"spd {wind['SIM_WIND_SPD']:.1f} m/s  "
...
def _worker_entry(args):
    (case_id, case_seed, config, perturbed, output_dir, workspace,
     speedup, save_plot, hard_timeout, params_file, wind) = args
...
SUMMARY_COLUMNS = [
    ...
    "wind_case", "wind_speed_mps", "wind_dir_deg", "wind_dir_z_deg",
...
def main():
    parser = argparse.ArgumentParser(
        description="Monte Carlo Runner — uSTOL all-FBWA + landing + WIND (mission_ustol_7). "
                    "Edit WIND_CASE at the top of this file to pick the wind condition.")
    ...
    # Wind: validate the selected case up front and resolve the per-case speeds.
    _valid_cases = {"head", "tail", "cross", "up", "down", "none"}
    if WIND_CASE.lower() not in _valid_cases:
        print(f"\nERROR: WIND_CASE={WIND_CASE!r} invalid; "
              f"use one of {sorted(_valid_cases)}")
        sys.exit(1)
    runway_hdg = config["mission"].get("runway_heading_deg", 285)
    levels = WIND_SPEED_LEVELS if WIND_CASE.lower() != "none" else [0.0]

    print("=" * 70)
    print("  LAT Monte Carlo Runner — uSTOL all-FBWA + landing + WIND (mission_ustol_7)")
    print("=" * 70)
    print(f"  Wind case     : {WIND_CASE}"
          f"{f'  ({CROSSWIND_SIDE})' if WIND_CASE.lower() == 'cross' else ''}")
    print(f"  Wind speeds   : {levels} m/s  (swept across cases)")
    ...
    if args.output_dir:
        ...
        # Tag with the wind case so the 5 campaigns stay separate.
        output_dir = str(SCRIPT_DIR / f"mc_{ts}_{WIND_CASE.lower()}")
    ...
    # Per-case wind: the campaign's WIND_CASE at the swept speed levels[cid % len].
    all_wind = {cid: wind_params_for_case(
                    WIND_CASE, runway_hdg, levels[cid % len(levels)], CROSSWIND_SIDE)
                for cid in range(num_runs)}
    ...
    print("\n" + "=" * 70)
    print(f"  SUMMARY — uSTOL ALL-FBWA + LANDING + WIND CAMPAIGN  (wind: {WIND_CASE})")
    print("=" * 70)
    ...
    with open(done_path, "w") as f:
        ...
        f.write(f"Wind case       : {WIND_CASE}  speeds {levels} m/s\n")
