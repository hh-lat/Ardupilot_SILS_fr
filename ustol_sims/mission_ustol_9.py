...
Sequence (mission_ustol_8 profile + differential thrust):
  ...
  6) POWERED FLARE: THROTTLE arrests the sink around a height-scheduled target; the elevator
     only holds a modest nose-up (power-off flare is impossible on this airframe). Touchdown,
     then idle rollout + DISARM.
...
FLARE_ALT     = 8.0                            # begin the flare below this height (m AGL)
...
K_THR_SINK    = 80.0                           # approach throttle gain: pwm per (m/s) of sink error
...
K_THR_FLARE   = 110.0                          # flare throttle gain: pwm per (m/s) of sink error
...
# 6. POWERED FLARE: THROTTLE arrests the sink around a height-scheduled target; the elevator only
#    holds a modest, achievable nose-up. Throttle is cut to idle ONLY after touchdown (step 7).
t = time.time(); t_prev = t; last = 0
while time.time() - t < 30:
    ...
    theta     = rate_limit(theta, TH_FLARE_HOLD, dt)         # ease to a modest, achievable nose-up
    ...
    if now - last > 1:
        print(f"FLARE t={now-t:4.0f}s cmd={theta:5.1f} pit={st['pitch']:5.1f} aoa={st['aoa']:4.1f} "
...
print("DONE (FBWA takeoff+climb -> %.0fs cruise hold -> %.0fs LOITER R=%.0fm%s -> approach %.0fm/s -> POWERED flare -> touchdown)"
