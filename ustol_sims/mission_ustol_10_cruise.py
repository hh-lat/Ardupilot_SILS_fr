...
"""
uSTOL stability driver (connects to an already-running SITL) - CRUISE ROLL/YAW DOUBLETS.

A single-run, watch-it-on-a-map sibling of mission_ustol_9.py, but its ONLY purpose is to check
that the aircraft is dynamically stable when perturbed in roll and in yaw at cruise. It reuses the
same FBWA + streamed-RC-override idiom and the same differential-thrust mixer setup, but it drops
the loiter and the landing: instead it flies a sequence of bank-angle and rudder doublets and ends
as soon as the aircraft has settled back to steady, wings-level cruise.

Sequence:
  0) claim GCS sysid + headless-arm params + roll/rudder limits, and (DIFF_THRUST) map the nine ESC
  ...
  5) YAW DOUBLETS: same +10 -> 0 -> -10 -> 0 -> +20 -> 0 -> -20 -> 0 pattern, but on RUDDER
     ...
     so this is an open-loop yaw kick: watch the yaw rate spike and then damp, the heading return
     toward trim, and - with DIFF_THRUST on - the motor1/motor9 split that the mixer adds.
...
See mission_ustol_9.py for the full notes; behaviour here is identical (yaw demand -> motor split).
